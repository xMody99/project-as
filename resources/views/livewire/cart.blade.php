<div>
    ...
    <li class="font-sans block mt-4 lg:inline-block lg:mt-0 lg:ml-6 align-middle text-black hover:text-gray-700">
        <a href="#" role="button" wire:click="changeStatus" class="relative flex">
        ...
        @if($showAll)
            <div class="absolute bg-gray-200 p-2 rounded">
                @if ($totalQuantity > 0)
                    <ul>
                        @foreach ($cart as $cartElement)
                            <li>{{$cartElement->product->name}}: {{$cartElement->quantity}}x{{$cartElement->product->price}}zł <button wire:click="delete({{$cartElement->id}})">Usuń</button></li>
                        @endforeach
                    </ul>
                    <b>Łącznie: {{$total}}zł</b>
                @else
